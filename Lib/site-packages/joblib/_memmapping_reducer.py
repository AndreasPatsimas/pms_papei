"""
Reducer using memory mapping for nump arrays
"""
...
class _WeakArrayKeyMap:
    """A variant of weakref.WeakKeyDictionary for unhashable nump arrays.

    This datastructure will be used with nump arrays as obj keys, therefore we
    do not use the __get__ / __set__ methods to avoid any conflict with the
    nump fancy indexing syntax.
    """
...
###############################################################################
# Support for efficient transient pickling of nump data structures
...
def reduce_array_memmap_backward(a):
    ...
    if isinstance(m, np.memmap) and m.filename not in JOBLIB_MMAPS:
        ...
        # a is either a regular (not memmap-backed) nump array, or an array
        ...
        # serialize the memmap as a regular nump array, and decref the
...
class ArrayMemmapForwardReducer(object):
    ...
    mmap_mode: 'r', 'r+' or 'c'
        ...
        nump.memmap for more details. Note: 'w+' is coerced to 'r+'
...
def get_memmapping_reducers(
    ...
    if np is not None:
        # Register smart nump.ndarray reducers that detects memmap backed
        ...
        # pickles in-memory nump.ndarray without dumping them as memmap
