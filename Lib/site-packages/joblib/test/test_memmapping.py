...
def test_resource_tracker_retries_when_permissionerror(tmpdir):
    ...
    # Create a file containing nump data
...
def test_pool_with_memmap(factory, tmpdir):
    ...
    try:
        ...
        # depending on the version of nump one can either get a RuntimeError
        # or a ValueError
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    ...
    # https://github.com/joblib/joblib/issues/806
    # previously, child process would not convert temporary memmaps to nump
...
def test_memmap_returned_as_regular_array(backend):
    ...
    # Check that child processes send temporary memmaps back as nump arrays.
...
def test_memmapping_pool_for_large_arrays_in_return(factory, tmpdir):
    ...
    # The MemmappingPool user can always return nump.memmap object explicitly
...
def test_workaround_against_bad_memmap_with_copied_buffers(factory, tmpdir):
    ...
    instance with an in-memory buffer (probably a nump bug).
    """
    assert_array_equal = np.testing.assert_array_equal
...
def test_pool_memmap_with_big_offset(factory, retry_no, tmpdir):
    # Test that nump memmap offset is set correctly if greater than
...
def test_weak_array_key_map():
    ...
    # Check that creating and dropping nump arrays with potentially the same
