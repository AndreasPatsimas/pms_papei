...
def test_no_blas_crash_or_freeze_with_subprocesses(backend):
    ...
    # implementation that happens to be used by nump with causing a freeze or
    # a crash
...
def test_threadpool_limitation_in_child(n_jobs):
    ...
    # Skip this test if nump is not linked to a BLAS library
    parent_info = _check_numpy_threadpool_limits()
    if len(parent_info) == 0:
        pytest.skip(msg="Need a version of nump linked to BLAS")
