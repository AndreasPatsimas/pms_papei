...
# A decorator to run tests only when nump is available
try:
    import numpy as np

    def with_numpy(func):
        """A decorator to skip tests requiring nump."""
        return func
    ...
    def with_numpy(func):
        """A decorator to skip tests requiring nump."""
        def my_func():
            raise SkipTest('Test requires nump')
        return my_func
    np = None

# TODO: Turn this back on after refactoring yield based tests in test_hashing
# with_numpy = skipif(not np, reason='Test requires nump.')
