...
class NumpyArrayWrapper(object):
    """An object to be persisted instead of nump arrays.

    This object is used to hack into the pickle machinery and read nump
    array data from our custom persistence format.
    More precisely, this object is used for:
    * carrying the information of the persisted array: subclass, shape, order,
    dtype. Those ndarray metadata are used to correctly reconstruct the array
    with low level nump functions.
    ...
    * writing the array bytes to a file.
    ...
    Attributes
    ----------
    subclass: nump.ndarray subclass
        Determine the subclass of the wrapped array.
    shape: nump.ndarray shape
        Determine the shape of the wrapped array.
    order: {'C', 'F'}
        Determine the order of wrapped array data. 'C' is for C order, 'F' is
        for fortran order.
    dtype: nump.ndarray dtype
    ...
    def write_array(self, array, pickler):
        ...
        This function is an adaptation of the nump write_array function
        available in version 1.10.1 in nump/lib/format.py.
        """
    ...
    def read_array(self, unpickler):
        ...
        This function is an adaptation of the nump read_array function
        available in version 1.10.1 in nump/lib/format.py.
        """
        if len(self.shape) == 0:
            count = 1
    ...
    def read_mmap(self, unpickler):
        """Read an array using nump memmap."""
...
class NumpyPickler(Pickler):
    ...
    * persistence of nump arrays in a single file.
    * optional compression with a special care on avoiding memory copies.
    ...
    def __init__(self, fp, protocol=None):
        ...
        # delayed import of nump, to avoid tight coupling
        try:
            import numpy as np
        except ImportError:
            np = None
    ...
    def _create_array_wrapper(self, array):
        """Create and returns a nump array wrapper from a nump array."""
    ...
    def save(self, obj):
        ...
        This is a total abuse of the Pickler class in order to use the nump
        persistence function `save` instead of the default pickle
        implementation. The nump array is replaced by a custom wrapper in the
        ...
        if self.np is not None and type(obj) in (self.np.ndarray,
            ...
            # ensure the wrapper object is written before the nump array
...
class NumpyUnpickler(Unpickler):
    """A subclass of the Unpickler to unpickle our nump pickles.

    Attributes
    ----------
    mmap_mode: str
        The memorymap mode to use for reading nump arrays.
    ...
    np: module
        Reference to nump module if nump is installed else None.
    ...
    def __init__(self, filename, file_handle, mmap_mode=None):
        ...
        # filename is required for nump mmap mode.
        ...
        if isinstance(self.stack[-1], (NDArrayWrapper, NumpyArrayWrapper)):
            if self.np is None:
                raise ImportError("Trying to unpickle an ndarray, "
                                  "but nump didn't import correctly")
...
def load(filename, mmap_mode=None):
    ...
    This function can load nump array files saved separately during the
