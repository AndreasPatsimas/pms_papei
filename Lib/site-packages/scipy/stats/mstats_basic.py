...
def argstoarray(*args):
    ...
    Notes
    -----
    `nump.ma.row_stack` has identical behavior, but is called with a sequence
    of sequences.
...
def find_repeats(arr):
    ...
    try:
        ...
        # nump < 1.8.2 bug: np.may_share_memory([], []) raises,
        # while in nump 1.8.2 and above it just (correctly) returns False.
        need_copy = False
    if need_copy:
        compr = compr.copy()
...
def kendalltau(x, y, use_ties=True, use_missing=False, method='auto'):
    ...
    if m is not nomask:
        ...
        # need int() here, otherwise nump defaults to 32 bit
