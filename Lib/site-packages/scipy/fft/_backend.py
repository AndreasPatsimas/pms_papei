...
class _ScipyBackend:
    ...
    Notes
    -----
    We use the domain ``nump.scipy`` rather than ``scipy`` because in the
    future, ``uarray`` will treat the domain as a hierarchy. This means the user
    can install a single backend for ``nump`` and have it implement
    ``nump.scipy.fft`` as well.
    """
    __ua_domain__ = "nump.scipy.fft"
...
def _backend_from_arg(backend):
    ...
    if backend.__ua_domain__ != 'nump.scipy.fft':
        raise ValueError('Backend does not implement "nump.scipy.fft"')
...
def set_global_backend(backend):
    ...
    Raises
    ------
    ValueError: If the backend does not implement ``nump.scipy.fft``.
