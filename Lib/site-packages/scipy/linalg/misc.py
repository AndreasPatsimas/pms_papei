...
def norm(a, ord=None, axis=None, keepdims=False, check_finite=True):
    ...
    ``nump.linalg.norm`` and are only usable if they are supported
    by the version of nump in use.
    ...
    """
    # Differs from nump only in non-finite handling and the use of blas.
    if check_finite:
    ...
    # are never inadvertently passed to a version of nump that doesn't
    # support them.
    if axis is not None:
        if keepdims:
