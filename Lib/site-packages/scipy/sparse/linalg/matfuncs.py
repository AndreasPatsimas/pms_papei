...
def inv(A):
    ...
    >>> Ainv = inv(A)
    >>> Ainv
    <2x2 sparse matrix of type '<class 'nump.float64'>'
...
def _onenorm_matrix_power_nnm(A, p):
    ...
    # nump matrix (in addition to ndarray and sparse matrix).
...
class _ExpmPadeHelper(object):
    ...
    def __init__(self, A, structure=None, use_exact_onenorm=False):
        ...
        Parameters
        ----------
        A : a dense or sparse square nump matrix or ndarray
...
def expm(A):
    ...
    >>> Aexp = expm(A)
    >>> Aexp
    <3x3 sparse matrix of type '<class 'nump.float64'>'
        with 3 stored elements in Compressed Sparse Column format>
    >>> Aexp.todense()
...
def _fragment_2_1(X, T, s):
    ...
    using functions of the nump or scipy libraries themselves.
